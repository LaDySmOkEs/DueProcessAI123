...
export default function DocumentAnalyzer() {
    ...
    const trackUsage = async () => {
        try {
            ...
            const currentUsage = user.ai_usage_count || 0;
            await User.updateMyUserData({ ai_usage_count: currentUsage + 1 });
        } catch (error) {
            ...
            try {
                ...
                if (isAudio || file.name.toLowerCase().endsWith('.m4a')) {
                    ...
                    if (documentType === 'contract') {
                        documentPrompt = `Analyze this contract document. Identify the parties, key obligations, any potential breaches of contract, and specified remedies. Structure the output according to the provided JSON schema, paying close attention to the 'contractual_analysis' section.`;
                        responseSchema = comprehensiveAnalysisSchema;
                    } else {
                        documentPrompt = `Analyze this legal document for constitutional violations, inconsistencies, and procedural issues. Focus on due process violations and provide detailed analysis, omitting the 'contractual_analysis' section.`;
                        responseSchema = JSON.parse(JSON.stringify(comprehensiveAnalysisSchema));
                        if (responseSchema.properties.ai_analysis.properties.contractual_analysis) {
                            delete responseSchema.properties.ai_analysis.properties.contractual_analysis;
                        }
                    }
                ...
                const contractualAnalysis = analysisResult.ai_analysis?.contractual_analysis;
                ...
                if (analysisResult.document_type === 'audio_evidence') {
                    ...
                    if (documentType === 'contract' && contractualAnalysis) {
                        docKeyFindings = contractualAnalysis.key_obligations?.map(o => o.obligation) || [];
                        docDueProcessViolations = [];
                    } else {
                ...
                const newDocRecord = await AnalyzedDocument.create({
                    document_name: file.name,
                    file_url,
                    file_type: file.type,
                    file_size: file.size,
                    ...
                    contract_breaches: contractualAnalysis?.potential_breaches?.map(b => b.breached_obligation) || [],
                    tags: tags.split(',').map(t => t.trim()).filter(t => t),
                    upload_method: uploadMode,
                    notes
                });

                results.push({ ...newDocRecord, ...analysisResult, id: newDocRecord.id });
                                        ...
                                        >
                                            ...
                                            <CardContent className="space-y-4">
                                                ...
                                                {!result.error && !result.quota_issue && result.ai_analysis && (
                                                    <>
                                                        {result.ai_analysis.contractual_analysis && (
                                                            <div className="p-3 bg-blue-50 rounded-lg">
                                                                <h4 className="font-semibold text-blue-800 mb-2">
                                                                    Contract Analysis
                                                                </h4>
                                                                <div className="space-y-3 text-sm">
                                                                    {result.ai_analysis.contractual_analysis.key_obligations?.length > 0 && (
                                                                        <div>
                                                                            <div className="font-medium text-blue-700">Key Obligations:</div>
                                                                            <ul className="list-disc pl-5 text-blue-600">
                                                                                {result.ai_analysis.contractual_analysis.key_obligations.map((item, i) => (
                                                                                    <li key={i}><strong>{item.party}:</strong> {item.obligation}</li>
                                                                                ))}
                                                                            </ul>
                                                                        </div>
                                                                    )}
                                                                    {result.ai_analysis.contractual_analysis.potential_breaches?.length > 0 && (
                                                                        <div>
                                                                            <div className="font-medium text-red-700">Potential Breaches Found:</div>
                                                                            <ul className="list-disc pl-5 text-red-600">
                                                                                {result.ai_analysis.contractual_analysis.potential_breaches.map((item, i) => (
                                                                                    <li key={i}><strong>{item.breaching_party}</strong> may have breached their obligation to "{item.breached_obligation}". Evidence: {item.evidence_from_document}</li>
                                                                                ))}
                                                                            </ul>
                                                                        </div>
                                                                    )}
                                                ...
                                                {!result.error && (
                                                    <div className="pt-4 border-t">
                                                        <Link to={`/LegalDraftsman?source_doc_id=${result.id}`}>
