...
const navigationSections = {
  ...
  navigator: {
    ...
    items: [
      ...
      {
        title: "Homeschool Legal Center",
        url: createPageUrl("HomeschoolNavigator"),
        icon: Home,
        color: "text-teal-600",
        description: "State-specific homeschooling laws"
      },
      // New: Grade-specific curriculum links
      {
        title: "Curriculum: K-5",
        url: createPageUrl("HomeschoolCurriculumK5"), 
        icon: BookOpen,
        color: "text-green-700",
        description: "Detailed curriculum guides for K-5 grades"
      },
      {
        title: "Curriculum: 6-8",
        url: createPageUrl("HomeschoolCurriculum68"),
        icon: BookOpen,
        color: "text-green-700",
        description: "Detailed curriculum guides for 6-8 grades"
      },
      {
        title: "Curriculum: 9-12",
        url: createPageUrl("HomeschoolCurriculum912"),
        icon: BookOpen,
        color: "text-green-700",
        description: "Detailed curriculum guides for 9-12 grades"
      },
      {
...
export default function Layout({ children, currentPageName }) {
  ...
  const [expandedSections, setExpandedSections] = useState(() => {
    ...
    // Iterate through all sections to determine if any of their items match the current URL
    for (const sectionKey in navigationSections) {
      if (navigationSections[sectionKey].items.some(item => location.pathname === item.url)) {
        initialExpandedState[sectionKey] = true; // Ensure the parent section of the active link is expanded
      }
    }
    return initialExpandedState;
  });

  // Use useEffect to automatically expand sections if a link within them becomes active
  useEffect(() => {
    setExpandedSections(prev => {
      ...
      for (const sectionKey in navigationSections) {
        if (navigationSections[sectionKey].items.some(item => location.pathname === item.url)) {
          if (!prev[sectionKey]) { 
            newState[sectionKey] = true; // Expand it
            changed = true;
          }
        }
      }
    ...
    <div className="min-h-screen flex w-full bg-slate-50 relative">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-80 bg-white border-r border-slate-200 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 flex flex-col ${
        ...
        <div className="border-t border-slate-200 p-4 bg-white">
          ...
          {/* Add Network Diagnostic Link */}
          <Link to={createPageUrl("NetworkDiagnostic")} className="block mb-4">
            <Button variant="outline" className="w-full flex items-center justify-center gap-2">
